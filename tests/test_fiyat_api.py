    ...
    async def get(self, url, params=None):
        ...
        if url == "https://prices.azure.com/api/retail/prices":
            return _SahteYanit(
                {
                    "Items": [_ORNEK_KAYIT],
                    "NextPageLink": "https://prices.azure.com/sayfa2",
                }
            )
...
def test_kayitlari_getir_sayfalamayi_takip_eder(monkeypatch):
    ...
    sonuclar = asyncio.run(
        kayitlari_getir("serviceName eq 'Virtual Machines'", onbellek_kullan=False)
    )
...
def test_kayitlari_getir_onbellekler(monkeypatch):
    ...
    assert (
        _SahteIstemci.cagri_sayisi == ilk_cagri_sayisi
    )  # ikinci cagri onbellekten donuyor
